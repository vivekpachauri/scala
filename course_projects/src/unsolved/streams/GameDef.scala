...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    // don't think I need to filter in this method, this could simply return all possible neighbors
    // filter (x=>terrain(x._1.b1) && terrain(x._1.b2))

    /**
     * Returns the list of positions reachable from the current block
     * which are inside the terrain.
     */
    def legalNeighbors: List[(Block, Move)] = neighbors filter (x=>terrain(x._1.b1) && terrain(x._1.b2))

    /**
     * Returns `true` if the block is standing.
     */
    def isStanding: Boolean = this.b1.x == this.b2.x && this.b1.y == this.b2.y
